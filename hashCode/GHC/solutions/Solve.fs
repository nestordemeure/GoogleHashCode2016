...
// tant qu'on a des items à charger
   // remplir drone
   // envoyer le drone
let rec giveOrders warehouse orderId adress cell (pWeights:int []) dronesList prodList charge consignes =
   ...
   | p::pq, drone::dq when drone.loadLeft < pWeights.[p] -> // TODO more efficient if we search for an item small enough to be put in the drone
      ...
      giveOrders warehouse orderId adress cell pWeights newDroneList prodList [] newConsignes
   | p::pq, drone::dq ->
      // ajouter p au drone actuel
      drone.loadLeft <- drone.loadLeft - pWeights.[p]
      giveOrders warehouse orderId adress cell pWeights dronesList pq (p::charge) consignes
...
let solution droneNumber deadLine maxLoad (productWeights:_[]) (warehouses:_[]) orders = 
   ...
   for order in orders do
      for kv in order.BookedProducts do
         ...
         lastDrone <- -1
         result <- giveOrders warehouse order.idO order.adress warehouse.cell productWeights dronesByDistance prodList [] result
